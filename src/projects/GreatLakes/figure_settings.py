...
#     cmap = mpl.cm.PuOr
    if var in ('T2_prj','Ts_prj','Tmin_prj','Tmax_prj','Tmean_prj','T_prj'):
      ...
      cmap = mpl.cm.PuOr if var in ('pet_obs',) else mpl.cm.BrBG
    elif var == 'zs':
...
#       clevs = np.linspace(-100,100,41); clbl = '%2.0f'  
    ...
    if var in ('ratio',): # snow (liquid water equivalent) 
      ...
      clevs = np.linspace(0,4,21); clbl = '%02.1f' # mm/day
#       clevs = np.linspace(1,2,21); clbl = '%3.2f' # mm/day
...
#       clevs = np.linspace(0,1.,21); clbl = '%3.2f' # mm/day      
...
#       elif season in ('summer','spring'): clevs = np.linspace(0,8,17); clbl = '%2.0f' # mm/day
      clevs = np.linspace(0,5,26); clbl = '%2.1f' # mm/day
#       clevs = np.linspace(2.,3.,21); clbl = '%3.2f' # mm/day
    ...
    elif var in ('SST','Ts','Tlake','Tslb'): # surface temperature / SST
    ...
    elif var in ('SST_global','Ts_global','T2_global',): # surface temperature / SST
      clevs = np.linspace(245,305,41); clbl = '%03.0f' # K
      cmap = mpl.cm.gist_ncar; cmap.set_over('white'); cmap.set_under('blue') # different from T2
      if var[:3]=='SST': lmsklnd = True # mask land for SST      
      ...
      clevs = np.linspace(260,280,21); clbl = '%03.0f' # K
  ...
  if figuretype is None:
    ...
    if subplot == (1,1):
      ...
      if cbar:
        ...
        if cbo == 'horizontal': 
          margins = dict(bottom=0.135, left=0.1, right=0.97, top=0.9, hspace=0.0, wspace=0.0)
