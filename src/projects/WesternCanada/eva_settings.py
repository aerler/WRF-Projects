...
def loadShapeFit(variable_atts=None, **kwargs):
  ''' wrapper for eva.eva.loadShapeEnsemble that sets experiment and variable lists '''
  if variable_atts is None: variable_atts = variables_rc  
  return eva.loadShapeFit(variable_atts=variable_atts, WRF_exps=WRF_exps, CESM_exps=CESM_exps, 
                          WRF_ens=WRF_ens, CESM_ens=CESM_ens, **kwargs)
def loadStationFit(variable_atts=None, default_constraints=None, **kwargs):
  ''' wrapper for eva.eva.loadStationEnsemble that sets experiment and variable lists etc. '''
  if variable_atts is None: variable_atts = variables_rc  
  if default_constraints is None: default_constraints = constraints_rc    
  return eva.loadShapeFit(variable_atts=variable_atts, default_constraints=default_constraints,
