...
ftopo = Dataset('/home/guido/cesm/data/cesmpiwrf1x1.cam2_6hr_TS_PRECT_10yr_nopole_NtoS_1yr.nc','r')

prect = ftopo.variables['PRECT'][:,:,:]
...
prectmp = ftopo.variables['PRECT'][:,:]
lonm1 = ftopo.variables['lon'][:]
lat = ftopo.variables['lat'][:]
prec=np.zeros((prectmp.shape[0],prectmp.shape[1]+1),np.float64)
prec[:,0:-1] = prectmp; prec[:,-1] = prectmp[:,0]
