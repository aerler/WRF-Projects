...
A simple script that reads WRF and NARR data and displays it in a proper geographic projection;
...
#  rng = [0,1,2,9,10,11]
  ...
  for n in xrange(len(rng)):
    ...
    NARRrain += NARR.prt(time=time).get().squeeze() * weight[n]
  data.append(CFSRrain); data.append(NARRrain) 
  CFSRlon, CFSRlat = np.meshgrid(CFSR.lon.values, CFSR.lat.values)
  lon.append(CFSRlon); lon.append(NARR.lon.get()) 
  lat.append(CFSRlat); lat.append(NARR.lat.get())
...
#  ax.set_xlabel('Longitude'); ax.set_ylabel('Latitude')
  ...
  # save figure to disk
#  f.savefig(folder+'AnnualPrecipRA.pdf', **sf) # save figure to pdf
#  print('\nSaved figure in '+folder+'AnnualPrecipRA.pdf')
  # show plots
  pyl.show()
